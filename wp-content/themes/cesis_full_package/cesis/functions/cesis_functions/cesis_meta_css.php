...
//////////////////////////////////////////////////////////////
	...
	if($custom_general_header == 'yes'){
	  ...
	  $header_units = $m_header_width["units"];
		...
		if($header_units !== '%'){
		$header_width = ($m_header_width["width"] + 80).$header_units;
		}else{
		$header_width = $m_header_width["width"];
		}
...
($header_v_pos == 'header_v_pos_left') ? $oc_pos = '' : $oc_pos = '-';
 ...
 $d_menu_space = ($cesis_data["cesis_menu_space"] / 2);
 $menu_type = redux_post_meta( 'cesis_data', get_the_ID(), 'cesis_meta_menu_type');
 $menu_space = (redux_post_meta( 'cesis_data', get_the_ID(), 'cesis_meta_menu_space') / 2);
 ...
 $d_header_a_space = ( $cesis_data["cesis_header_additional_space"] / 2 );
 ...
 $header_a_space = ( redux_post_meta( 'cesis_data', get_the_ID(), 'cesis_meta_header_additional_space') / 2 );
...
{height:'.esc_attr($header_shrinked_height).'px;}';
 ...
 .header_sub .nav_bottom_borderx.text_border  > div > ul > li > a span{padding-bottom:'.esc_attr(5+$menu_border_size).'px;}';
...
/* mobile breakpoint */
 $meta_css .= '@media only screen and (min-width: '.esc_attr($header_breakpoint+1).'px) {
...
if($title_width_units !== '%'){
  $title_width = ($title_width_array["width"] + 80).$title_width_units;
}else{
...
$content_width = 100 - redux_post_meta( 'cesis_data', get_the_ID(), "cesis_meta_sidebar_size");

$d_sidebar_space = $cesis_data["cesis_sidebar_space"] / 2;
$sidebar_space = redux_post_meta( 'cesis_data', get_the_ID(), "cesis_meta_sidebar_space") / 2;
...
$page_area_width = ($meta_page_content_width["width"] + 80).$page_content_width_units;
}else{
...
$blog_area_width = ($bcw["width"] + 80).$blog_content_width_units;
}else{
...
$portfolio_area_width = ($pcw["width"] + 80).$portfolio_content_width_units;
}else{
...
$custom_staff_page = redux_post_meta( 'cesis_data', get_the_ID(), "cesis_meta_staff_custom_layout" );
  ...
  $staff_area_width = ($scw["width"] + 80).$staff_content_width_units;
  }else{
...
$custom_career_page = redux_post_meta( 'cesis_data', get_the_ID(), "cesis_meta_career_custom_layout" );
  ...
  $career_area_width = ($ccw["width"] + 80).$career_content_width_units;
  }else{
...
$d_footer_main_width = ($cesis_data["cesis_footer_width"]["width"] + 80).$d_footer_main_width_units;
}else{
...
$footer_main_width = ($footer_main_width["width"] + 80).$footer_main_width_units;
}else{
...
$d_footer_sub_width = ($cesis_data["cesis_footer_bar_width"]["width"] + 80).$d_footer_sub_width_units;
}else{
...
$footer_sub_width = ($footer_sub_width["width"] + 80).$footer_sub_width_units;
}else{
